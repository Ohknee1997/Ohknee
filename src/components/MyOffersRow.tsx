...
export const MyOffersRow: React.FC<MyOffersRowProps> = ({
  ...
  return (
    <section className="w-full rounded-2xl bg-[#131722] border border-[#242b3d] mb-4 shadow-lg overflow-hidden transition-all hover:border-purple-500/50 select-none">
      {/* Top Banner Row - Click to Toggle */}
      <div
        onClick={() => setIsExpanded(!isExpanded)}
        className="h-[80px] sm:h-[88px] min-h-[80px] sm:min-h-[88px] px-3.5 sm:px-5 py-3 flex items-center justify-between gap-3 sm:gap-4 cursor-pointer group"
      >
        ...
        <div className="flex-1 sm:flex-initial sm:w-[280px] md:w-[320px] flex items-center gap-3 sm:gap-3.5 min-w-0">
          <div className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-2xl bg-amber-500/20 border border-amber-400/40 text-amber-300 group-hover:scale-105 transition-all flex-shrink-0 shadow-xs">
        ...
        <div className="hidden md:flex flex-1 items-center justify-start gap-2.5 sm:gap-3 overflow-hidden px-2 sm:px-3">
          {displayOffers.slice(0, 5).map((offer) => {
            ...
            return (
              <button
                ...
                className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-[#181d2c] border border-slate-800 p-2 flex-shrink-0 shadow-xs hover:border-purple-400 transition-all cursor-pointer"
          ...
          {displayOffers.length > 5 && (
            <div className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-[#1c2233] border border-slate-800 text-xs font-black text-purple-300 flex-shrink-0">
              +{displayOffers.length - 5}
            </div>
          )}
        </div>
        ...
        <div className="flex-shrink-0 flex items-center justify-end pl-2">
          <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[#1c2233] text-purple-300 border border-slate-700/60 text-xs font-bold group-hover:bg-purple-950 transition-colors">
      ...
      {/* Expanded Grid View */}
      {isExpanded && (
        <div className="p-3 sm:p-4 border-t border-[#242b3d] bg-[#0d0f15]/80 animate-in fade-in duration-200">
